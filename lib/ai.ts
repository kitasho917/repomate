 ...
 */
export async function reviseReportWithInstruction(reportBody: string, instruction: string): Promise<string> {
  ...
  if (normalized.includes("短") || normalized.includes("要約") || normalized.includes("short")) {
  ...
  if (normalized.includes("大学生") || normalized.includes("自然") || normalized.includes("student")) {
    return `${tagged}\n\n[AI提案] 普通の大学生が提出する自然な文章トーンに調整した版です。`;
  }

  if (normalized.includes("理系") || normalized.includes("学術") || normalized.includes("science")) {
    return `${tagged}\n\n[AI提案] 理系レポート向けに客観表現・手順の明確性を強めた版です。`;
  }

  if (normalized.includes("誤字") || normalized.includes("校正") || normalized.includes("proof")) {
