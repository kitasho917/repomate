...
export function AiAssistPanel({
  ...
  const sendInstruction = async (value: string) => {
    ...
    await onRequest(text);

    setChat((prev) => [
      ...prev,
      {
        role: "assistant",
        text: "修正案を作成しました。下のプレビューを確認してください。",
      },
    ]);
    setInstruction("");
    setLoading(false);
  ...
  return (
    <section className="card">
      ...
      <p className="mt-1 text-xs text-slate-600">
        本文への指示を自由入力できます（例: 考察を詳しくして、短くして）。
      </p>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {presets.map((preset) => (
          <button
            ...
            onClick={() => void sendInstruction(preset.key)}
          >
            {preset.label}
          </button>
        ))}
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button
          ...
          onClick={() => void sendInstruction(instruction)}
          disabled={loading}
        >
          {loading ? "生成中..." : "送信"}
        </button>
      ...
      <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-2">
        ...
        <pre className="mt-1 max-h-40 overflow-auto whitespace-pre-wrap text-xs text-emerald-900">
          {suggestion || "まだ修正案はありません。"}
        </pre>
        <button
